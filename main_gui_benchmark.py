...
class TestingGUI:
    ...
    def setup_frames(self, is_plotting_sheet: bool = False) -> None:
        ...
        # Instead of destroying the entire frames, clear their children if they exist.
        for attr in ['table_frame', 'plot_frame', 'image_frame']:
            if hasattr(self, attr):
                ...
                if frame is not None:
                    ...
                    frame.pack_forget()
                # Safely set the attribute to None
                setattr(self, attr, None)

        # Create new frames based on sheet type
        if is_plotting_sheet:
            # Plotting sheet layout
            ...
            self.image_frame.pack(side="bottom", fill="x", expand=False, padx=5, pady=5)
        else:
            # Non-plotting sheet layout
            self.table_frame = ttk.Frame(self.display_frame)
        ...
        print(f"Frames set up: plotting={is_plotting_sheet}")
    ...
    def update_displayed_sheet(self, sheet_name: str) -> None:
        ...
        print("DEBUG: Checking if display_frame exists before accessing children...")
        ...
        #  **Step 4: Now it is safe to create ImageLoader**
    ...
    def load_images(self, is_plotting_sheet):
        """Load and display images in the UI."""
        if not hasattr(self, 'image_frame') or not self.image_frame.winfo_exists():
            print("Error: Image frame does not exist, recreating...")
            self.image_frame = ttk.Frame(self.display_frame)
    ...
    def clear_display_frame(self, is_plotting_sheet: bool, is_empty_sheet: bool) -> None:
        ...
        """
        print(f"DEBUG: Clearing display frame (is_plotting_sheet={is_plotting_sheet}, is_empty_sheet={is_empty_sheet})")

        # Debug existing frames before destruction
        print("DEBUG: Existing frames before clearing:")
        if hasattr(self, 'table_frame'):
            print(f"  table_frame: {self.table_frame}")
        if hasattr(self, 'plot_frame'):
            print(f"  plot_frame: {self.plot_frame}")
        if hasattr(self, 'image_frame'):
            print(f"  image_frame: {self.image_frame}")

        # Clear the table frame
        if hasattr(self, 'table_frame'):
            print("DEBUG: Clearing table_frame contents...")
            for widget in self.table_frame.winfo_children():
                widget.destroy()

        # Clear the plot frame if it's a plotting sheet
        if hasattr(self, 'plot_frame') and self.plot_frame:
            print("DEBUG: Destroying plot_frame...")
            for widget in self.plot_frame.winfo_children():
                widget.destroy()
            self.plot_frame.destroy()
            self.plot_frame = None

        # Ensure image_frame is not removed prematurely
        if hasattr(self, 'image_frame') and self.image_frame:
            print("DEBUG: Clearing image_frame contents...")
            for widget in self.image_frame.winfo_children():
                widget.destroy()

        print("DEBUG: Finished clearing frames. Recreating based on sheet type...")

        self.display_frame.update_idletasks()
        self.root.update_idletasks()

        # Recreate frames based on plotting sheet status
        if is_plotting_sheet:
            self.table_frame = ttk.Frame(self.display_frame)
            ...
            print("DEBUG: Created new table_frame and plot_frame for plotting sheet.")
        else:
            self.table_frame = ttk.Frame(self.display_frame)
            self.table_frame.pack(fill="both", expand=True)

            print("DEBUG: Created new table_frame for non-plotting sheet.")

        

        print("DEBUG: Created new image_frame.")

        # If the sheet is empty, display a message
        if is_empty_sheet:
            print("DEBUG: Sheet is empty. Displaying message.")
            empty_label = tk.Label(
                self.display_frame,
        ...
        print("DEBUG: Finished setting up frames after clearing.")
    ...
    def display_table(self, frame, data, sheet_name, is_plotting_sheet=False):
        ...
        load_image_button = ttk.Button(button_frame, text="Load Images", command=lambda: self.image_loader.add_images())
        load_image_button.pack(anchor='center')
    ...
    def generate_full_report(self):
        ...
        try:
            # Pass the current filtered sheets and plot options.
            self.report_generator.generate_full_report(self.filtered_sheets, self.plot_options)
        finally:
